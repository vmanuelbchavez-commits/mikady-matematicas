...
function GestionClasesColegio() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      setFormData({ titulo: '', descripcion: '', grado: '1' })
      setArchivo(null)
      cargarClases()
    }
    setLoading(false)
  ...
  const eliminarClase = async (id) => {
    if (confirm('¿Estás segura de eliminar esta clase?')) {
      await supabase.from('clases_colegio').delete().eq('id', id)
      cargarClases()
    }
  }
  ...
  return (
    <div className="admin-page">
      <header className="page-header">
        ...
        <h1>🏫 Gestionar Clases del Colegio</h1>
      </header>
          ...
          <div className="items-list">
            ...
            {clases.map((clase) => (
              <div key={clase.id} className="item-card">
                ...
                <p><strong>Grado:</strong> {clase.grado}º Primaria</p>
...
export default GestionClasesColegio
